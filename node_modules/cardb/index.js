...
var stm=db.prepare('select *,rowid from pre order by sku desc');
try{var row=stm.all();
}catch(err){console.log(err);}
return row;
};

exports.skuPre=function(sku){
var stm=db.prepare('select * from pre where sku=?');
try{var row=stm.get(sku);
}catch(err){console.log(err);}
return row;
};

exports.insPre=function(name,sku,pri,img,rel,cat,des){
var stm=db.prepare('insert into pre values(?,?,?,?,?,?,?)');
stm.run(name,sku,pri,img,rel,cat,des);
};

exports.delPre=function(sku){
var stm=db.prepare('delete from pre where sku=?')
stm.run(sku)
};

exports.namPre=function(name,sku){
var stm=db.prepare('update pre set name=? where sku=?')
try{stm.run(name,sku)}
catch(err){console.log(err)}
};

exports.priPre=function(pri,sku){
var stm=db.prepare('update pre set pri=? where sku=?')
stm.run(pri,sku)
};

exports.colPre=function(col,sku){
var stm=db.prepare('update pre set col=? where sku=?')
stm.run(col,sku)
};

exports.catPre=function(cat,sku){
var stm=db.prepare('update pre set cat=? where sku=?')
stm.run(cat,sku)
};

exports.desPre=function(des,sku){
var stm=db.prepare('update pre set des=? where sku=?')
stm.run(des,sku)
};

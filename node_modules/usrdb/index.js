...
var stm=db.prepare('insert into pid values(?,?,?,?,?,?,0)');
stm.run(email,pid,mnt,buy,ite,tim)
};

exports.getPid=function(pid){
...
exports.insQR=function(pid,qr,act){
var stm=db.prepare('insert into qr values(?,?,?)');
stm.run(pid,qr,act);
};

exports.delQR=function(pid){
var stm=db.prepare('delete from qr where pid=?')
stm.run(pid);
};
...
exports.pdelQR=function(pid){
var stm=db.prepare('delete from qr where id=?');
stm.run(pid);
};
// === pal ===

exports.insPal=function(email,id,mnt,ite,utc,act){
var stm=db.prepare('insert into pal values(?,?,?,?,?,?)');

try{stm.run(email,id,mnt,ite,utc,0)}
catch(err){console.log(err)}
};
